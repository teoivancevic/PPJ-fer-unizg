...
//                 if (paramType == "int") paramTypes.push_back(TypeInfo(BasicType::INT));
//                 else if (paramType == "char") paramTypes.push_back(TypeInfo(BasicType::CHAR));
//                 else if (paramType == "void") paramTypes.push_back(TypeInfo(BasicType::VOID));
//             }
//             BasicType returnType = BasicType::VOID;
//             if (entry->type == "int") returnType = BasicType::INT;
//             else if (entry->type == "char") returnType = BasicType::CHAR;
//             node->typeInfo = TypeInfo::makeFunctionType(returnType, paramTypes);
//         }
...
//         node->typeInfo = TypeInfo::makeArrayType(BasicType::CHAR, true);
//         node->isLValue = false;
//     }
//     else {
//         reportError(node);
...
void IzrazProcessor::process_postfiks_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_lista_argumenata(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_unarni_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_unarni_operator(Node* node) {
...
void IzrazProcessor::process_cast_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_ime_tipa(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_specifikator_tipa(Node* node) {
    if (!node || node->children.size() != 1) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_multiplikativni_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_aditivni_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_odnosni_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_jednakosni_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_bin_i_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_bin_xili_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_bin_ili_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_log_i_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
...
void IzrazProcessor::process_log_ili_izraz(Node* node) {
    if (!node) {
        reportError(node);
        return;
    }
