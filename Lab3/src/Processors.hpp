...
{
public:
    Processor(SymbolTable *scope) : currentScope(scope) {}
...
public:
    ...
    void process_log_ili_izraz(Node *node);       // done
    void process_izraz_pridruzivanja(Node *node); // done
    void process_izraz(Node *node);               // done
};
...
public:
    using Processor::Processor;
    // Svaka processor klasa bi trebala imati svoj validator i scope
    class Validator
    {
    public:
        static bool validateNumConstant(const string &lexeme, TypeInfo &outType)
